...
curl --fail-with-body -i -H "Authorization: token $GITHUB_TOKEN" \
  ...
  -X POST https://api.github.com/repos/"$GITHUB_REPOSITORY_OWNER"/eclipse-cs/issues
...
curl --fail-with-body -i -H "Authorization: token $GITHUB_TOKEN" \
  ...
  -X POST https://api.github.com/repos/"$GITHUB_REPOSITORY_OWNER"/sonar-checkstyle/issues
